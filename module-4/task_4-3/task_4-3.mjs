...
const WGTypes = [
  { value: 1, caption: "Donflamingo" },
  { value: 2, caption: "Sir Crocodille" },
  { value: 3, caption: "Kuma" },
  { value: 4, caption: "Boa Hancock" },
  { value: 5, caption: "Jinbei" },
  { value: 6, caption: "Mihawk" },
  { value: 7, caption: "Gecko Moria" },
  { value: 8, caption: "Trafalger D Water Law" },
  { value: 9, caption: "Weevile" },
  { value: 10, caption: "Buggy D Clown" },
  { value: 11, caption: "Sakazuki" },
  { value: 12, caption: "Borsalino" },
  { value: 13, caption: "Kuzan" },
];
...
function rdbCarNameSelect(aEvent){
  const txtWGOutput = document.getElementById("txtTask4Output");
  txtTask4Output.innerHTML =
    `User select fella number: ${aEvent.target.value}`
  ;
} 

const divMarines = document.getElementById("divTask4Cars");
for(let i = 0; i < WGTypes.length; i++){
  const marine = WGTypes[i];
  const inpRadio = document.createElement("input");
  inpRadio.type = "radio";
  inpRadio.name = "rdbMarineName";
  inpRadio.value = marine.value;
  inpRadio.id = "rdbMarineName" + i.toString();
  inpRadio.addEventListener("change", rdbCarNameSelect);
  const lblCaption = document.createElement("label");
  lblCaption.for = inpRadio.id;
  lblCaption.appendChild(document.createTextNode(marine.caption));
  divMarines.appendChild(inpRadio);
  divMarines.appendChild(lblCaption);
  divMarines.appendChild(document.createElement("br"));

  console.log(`MarineTypes[${i}].value = ${marine.value}, MarineTypes[${i}].caption = ${marine.caption}`)
}
//--- Part 5 ----------------------------------------------------------------------------------------------
/* Put your code below here!*/
